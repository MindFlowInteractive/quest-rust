//! Input module.
//!
//! Captures and normalises player input for puzzle interaction. Maps
//! command-line/keyboard input streams into structured [`GameAction`] values.

use crate::errors::AppError;
use std::io::BufRead;

/// Structured representation of actions a player can take in the game.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GameAction {
    /// Selecting a specific puzzle option or index.
    Select(usize),
    /// Confirming an action or accepting a choice.
    Confirm,
    /// Returning to the previous screen or canceling.
    Back,
    /// Exiting the game or puzzle session.
    Exit,
    /// Asking for help or instructions.
    Help,
}

/// Normalises and parses a raw string input into a [`GameAction`].
///
/// Leading and trailing whitespaces are trimmed, and the text is converted
/// to lowercase before matching.
pub fn parse_input(input: &str) -> Result<GameAction, AppError> {
    let normalised = input.trim().to_lowercase();
    if normalised.is_empty() {
        return Err(AppError::InputEmpty);
    }

    match normalised.as_str() {
        "confirm" | "yes" | "y" | "ok" | "agree" | "submit" => Ok(GameAction::Confirm),
        "back" | "no" | "n" | "cancel" | "return" => Ok(GameAction::Back),
        "exit" | "quit" | "q" => Ok(GameAction::Exit),
        "help" | "h" | "info" | "?" => Ok(GameAction::Help),
        other => {
            // Attempt to parse standalone numeric selection
            if let Ok(num) = other.parse::<usize>() {
                return Ok(GameAction::Select(num));
            }

            // Attempt to parse option prefixed with 'select'
            if let Some(stripped) = other.strip_prefix("select ")
                && let Ok(num) = stripped.trim().parse::<usize>()
            {
                return Ok(GameAction::Select(num));
            }

            Err(AppError::InputInvalid(input.to_string()))
        }
    }
}

/// Captures a single line of input from a [`BufRead`] reader source,
/// normalises it, and parses it into a [`GameAction`].
pub fn read_action<R: BufRead>(reader: &mut R) -> Result<GameAction, AppError> {
    let mut buffer = String::new();
    reader.read_line(&mut buffer)?;

    parse_input(&buffer)
}

#[cfg(test)]
mod tests {
    use super::*;
    use std::io::Cursor;

    #[test]
    fn test_parse_valid_confirm() {
        assert!(matches!(parse_input("confirm"), Ok(GameAction::Confirm)));
        assert!(matches!(parse_input("  YES  "), Ok(GameAction::Confirm)));
        assert!(matches!(parse_input("y"), Ok(GameAction::Confirm)));
        assert!(matches!(parse_input("ok"), Ok(GameAction::Confirm)));
        assert!(matches!(parse_input("submit"), Ok(GameAction::Confirm)));
    }

    #[test]
    fn test_parse_valid_back() {
        assert!(matches!(parse_input("back"), Ok(GameAction::Back)));
        assert!(matches!(parse_input("  no  "), Ok(GameAction::Back)));
        assert!(matches!(parse_input("n"), Ok(GameAction::Back)));
        assert!(matches!(parse_input("cancel"), Ok(GameAction::Back)));
        assert!(matches!(parse_input("return"), Ok(GameAction::Back)));
    }

    #[test]
    fn test_parse_valid_exit() {
        assert!(matches!(parse_input("exit"), Ok(GameAction::Exit)));
        assert!(matches!(parse_input("QUIT"), Ok(GameAction::Exit)));
        assert!(matches!(parse_input("q"), Ok(GameAction::Exit)));
    }

    #[test]
    fn test_parse_valid_help() {
        assert!(matches!(parse_input("help"), Ok(GameAction::Help)));
        assert!(matches!(parse_input("?"), Ok(GameAction::Help)));
        assert!(matches!(parse_input("info"), Ok(GameAction::Help)));
    }

    #[test]
    fn test_parse_valid_select() {
        assert!(matches!(parse_input("1"), Ok(GameAction::Select(1))));
        assert!(matches!(parse_input("42"), Ok(GameAction::Select(42))));
        assert!(matches!(parse_input("select 3"), Ok(GameAction::Select(3))));
        assert!(matches!(parse_input("  select   5  "), Ok(GameAction::Select(5))));
    }

    #[test]
    fn test_parse_empty_input() {
        assert!(matches!(parse_input(""), Err(AppError::InputEmpty)));
        assert!(matches!(parse_input("   \n\r "), Err(AppError::InputEmpty)));
    }

    #[test]
    fn test_parse_invalid_inputs() {
        let result = parse_input("invalid");
        assert!(matches!(result, Err(AppError::InputInvalid(_))));
        assert!(result.unwrap_err().to_string().contains("invalid"));

        let result = parse_input("select abc");
        assert!(matches!(result, Err(AppError::InputInvalid(_))));
        assert!(result.unwrap_err().to_string().contains("select abc"));

        let result = parse_input("confirm please");
        assert!(matches!(result, Err(AppError::InputInvalid(_))));
        assert!(result.unwrap_err().to_string().contains("confirm please"));
    }

    #[test]
    fn test_read_action_from_stream() {
        let mut input = Cursor::new("yes\n  select 4  \nquit\ninvalid\n");

        assert!(matches!(read_action(&mut input), Ok(GameAction::Confirm)));
        assert!(matches!(read_action(&mut input), Ok(GameAction::Select(4))));
        assert!(matches!(read_action(&mut input), Ok(GameAction::Exit)));
        assert!(matches!(
            read_action(&mut input),
            Err(AppError::InputInvalid(_))
        ));
        assert!(matches!(read_action(&mut input), Err(AppError::InputEmpty)));
    }
}
